...
def test_greetings_info():
    ...
    with patch('src.views.greeting', return_value=mock_greeting) as mock_greet, \
            patch('src.views.get_cards_info', return_value=mock_cards_info) as mock_cards, \
            patch('src.views.get_top_5_transactions', return_value=mock_top_transactions) as mock_top_transactions, \
            patch('src.views.fetch_currency_rate', return_value=mock_currency_rates) as mock_currency, \
            patch('src.views.fetch_s_p_500_stock', return_value=mock_stock_prices) as mock_stocks:
